...
def invert_latents(unet, conditions_model, noise_scheduler, 
                    latents, conditions, conditions_keys_ordered,
                    free_guidance_ratio=7.5, num_inner_steps=10, early_stop_epsilon=1e-8,
...
def load_results(noisy_latents_path, uncond_embeddings_path):
    noisy_latents = np.load(noisy_latents_path)
    noisy_latents = torch.from_numpy(noisy_latents)

    uncond_embeddings = None
    if os.path.exists(uncond_embeddings_path):
        __uncond_embeddings = np.load(uncond_embeddings_path)
        uncond_embeddings = []
        ...
        print(f"WARNING: uncond_embeddings file {uncond_embeddings_path} not found, returning None for uncond_embeddings")
    return noisy_latents, uncond_embeddings

def save_results(noisy_latents, uncond_embeddings, path_name_latents, path_name_uncond_emb):
...
def create_save_load_null_inversion_results(models, null_inv_model_path, latents_from, conditions_from, s_id, s_session_id,
                                fgr=2.0,  inner_steps=2, early_stop_epsilon=1e-8):

    diff_steps = len(models["noise_scheduler"].timesteps)
    save_results_path = os.path.join(null_inv_model_path, f"diff_steps_{diff_steps}", f"fgr_{fgr}", f"inner_steps_{inner_steps}", s_id, s_session_id)
    noisy_latents_path = os.path.join(save_results_path, f"{s_id}_{s_session_id}_noisy_latents.npy")
    uncond_embeddings_path = os.path.join(save_results_path, f"{s_id}_{s_session_id}_uncond_embeddings.npy")


    if os.path.exists(noisy_latents_path) and (os.path.exists(uncond_embeddings_path) or inner_steps==0):
        # load
        noisy_latents, uncond_embeddings = load_results(noisy_latents_path, uncond_embeddings_path)
        ...
        inverted_data = invert_latents(
            ...
            latents_from,
            conditions_from,
            models["model_description"].args_train.conditions_keys_ordered,
            free_guidance_ratio=fgr,
            compute_uncond_embeddings=inner_steps>0,
        ...
        save_results(inverted_data["noisy_latents"], inverted_data["uncond_embeddings"], noisy_latents_path, uncond_embeddings_path)
        ...
        # save as json
        with open(os.path.join(save_results_path, f"{s_id}_{s_session_id}_description.json"), 'w') as f:
